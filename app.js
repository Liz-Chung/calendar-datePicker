...
const isParentInClass = (element, className) => {
    ...
    if(element.classList.contains(className) || element.classList.contains("calendar")) return true;

    if(element.parentElement) return isParentInClass(element.parentElement, className);
    return false;
};
